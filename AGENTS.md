# Orchestra — OpenAI Codex Guide

Orchestra is a Python multi-agent orchestration framework installed in this project.
`OPENAI_API_KEY` is already in your environment, so you can use Orchestra immediately
...
provider = auto_provider()  # picks up OPENAI_API_KEY automatically
...
# OpenAI (default when OPENAI_API_KEY is set — already the case in Codex)
