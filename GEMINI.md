# Orchestra — Gemini CLI Guide

Orchestra is a Python multi-agent orchestration framework installed in this project.
`GOOGLE_API_KEY` is already in your environment, so you can use Orchestra immediately
...
provider = auto_provider()  # picks up GOOGLE_API_KEY automatically
...
# Google (default when GOOGLE_API_KEY is set — already the case in Gemini CLI)
