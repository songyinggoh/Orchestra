...
from orchestra.core.types import Message, ToolCall, AgentResult
from orchestra.core.errors import AgentRevokedException, CapabilityDeniedError
from orchestra.security.acl import ToolACL
from orchestra.identity.agent_identity import AgentIdentity, RevocationList, AgentIdentityValidator
from orchestra.identity.types import UCANToken, UCANCapability
from orchestra.memory.serialization import unpack, pack
...
async def test_ucan_narrowing_enforcement_integration():
    ...
    child_caps_valid = (UCANCapability(resource="orchestra:tools/web_search", ability="tool/invoke"),)
    ...
    child_caps_invalid = (UCANCapability(resource="orchestra:tools/file_read", ability="tool/invoke"),)
...
async def test_agent_revocation_enforcement_integration():
    ...
    assert acl.is_authorized("any_tool", agent_did="did:peer:2:safe_agent", revocation_list=rl) is True
