...
import pytest
from examples.handoff import billing_agent, technical_agent, triage_agent
from examples.hitl_review import editor_agent, writer_agent
...
class TestAdvancedExamples:
    ...
    async def test_handoff_integration(self) -> None:
        ...
        provider = ScriptedLLM(
            [LLMResponse(content="technical issue"), LLMResponse(content="fixed")]
        )
    ...
    async def test_hitl_integration(self) -> None:
        ...
        provider = ScriptedLLM([LLMResponse(content="draft"), LLMResponse(content="polished")])
        ...
        final_state = await compiled.resume(
            run_id, state_updates={"feedback": "ok"}, event_store=store, provider=provider
        )
    ...
    async def test_time_travel_fork_integration(self) -> None:
        ...
        new_id, fork_state, start_node = await compiled.fork(
            "orig", 2, state_overrides={"input": "Topic B"}, event_store=store
        )
        ...
        final_fork = await compiled.run(
            fork_state, run_id=new_id, provider=p2, event_store=store, start_at=start_node
        )
