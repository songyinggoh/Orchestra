...
from orchestra.routing.types import SelectionFallback, SLAConstraint, BudgetConstraint

@pytest.mark.asyncio
async def test_select_model_within_budget_and_sla():
    router = CostAwareRouter()
    options = [
        ModelOption("smart", "p1", input_cost_1k=0.01, output_cost_1k=0.03, latency_score=4, capability_score=5),
        ModelOption("fast", "p2", input_cost_1k=0.001, output_cost_1k=0.002, latency_score=1, capability_score=3),
        ModelOption("mid", "p3", input_cost_1k=0.005, output_cost_1k=0.01, latency_score=2, capability_score=4),
    ]
...
async def test_fail_fast_raises_on_conflict():
    router = CostAwareRouter()
    options = [
        ModelOption("smart", "p1", input_cost_1k=0.1, output_cost_1k=0.3, latency_score=4, capability_score=5),
    ]
...
async def test_favor_cost_relaxes_sla():
    ...
    options = [
        ModelOption("smart", "p1", input_cost_1k=0.01, output_cost_1k=0.03, latency_score=4, capability_score=5),
        ModelOption("cheap", "p2", input_cost_1k=0.001, output_cost_1k=0.001, latency_score=2, capability_score=2),
    ]
...
async def test_favor_latency_relaxes_budget():
    router = CostAwareRouter()
    options = [
        ModelOption("fast-expensive", "p1", input_cost_1k=0.02, output_cost_1k=0.02, latency_score=1, capability_score=3),
        ModelOption("slow-cheap", "p2", input_cost_1k=0.001, output_cost_1k=0.001, latency_score=5, capability_score=3),
    ]
    ...
    # Still 0.02 > 0.015. So it will fall back to just picking fastest among all options if still empty.
