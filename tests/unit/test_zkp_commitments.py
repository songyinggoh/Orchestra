...
def test_state_commitment_chain():
    ...
    assert StateCommitment.verify(state1, res1.commitment, res1.nonce, previous_commitment=res0.commitment) is True
    
    # Wrong previous commitment should fail
    assert StateCommitment.verify(state1, res1.commitment, res1.nonce, previous_commitment=b"wrong") is False
