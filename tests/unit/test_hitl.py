...
async def test_interrupt_before() -> None:
    ...
    assert (
        final_state.get("__metadata__") is None
        or final_state["__metadata__"].get("status") == "completed"
    )
...
async def test_resume_with_state_updates() -> None:
    ...
    final_state = await compiled.resume(run_id, state_updates={"input": 50}, event_store=store)
