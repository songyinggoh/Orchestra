...
async def test_attenuate_token_delegation():
    ...
    caps = [UCANCapability("orchestra:tools", "invoke"), UCANCapability("orchestra:secrets", "read")]
    
    token = await attenuator.attenuate_token(
        context, ucan_mgr, "parent-jwt", "child-did", caps
    )
