...
from orchestra.core.types import ToolCall, ToolResult
...
async def test_tool_acl_patterns() -> None:
    ...
    assert (
        "executed safe_tool_1"
        in (
            await agent._execute_tool(ToolCall(id="1", name="safe_tool_1", arguments={}), context)
        ).content
    )
    assert (
        "executed safe_tool_2"
        in (
            await agent._execute_tool(ToolCall(id="2", name="safe_tool_2", arguments={}), context)
        ).content
    )
    assert (
        "Security Policy Violation"
        in (
            await agent._execute_tool(ToolCall(id="3", name="unsafe_tool", arguments={}), context)
        ).error
    )
