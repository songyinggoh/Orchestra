...
from orchestra.cost.tenant import BudgetConfig, BudgetState, BudgetStatus
...
class CyclicHierarchyError(Exception):
    ...
    def __init__(self, tenant_id: str, cycle_path: list[str]) -> None:
        ...
        super().__init__(
            f"Cyclic tenant hierarchy detected involving {tenant_id!r}: {path_str}"
        )
        self.tenant_id = tenant_id
...
class PersistentBudgetStore:
    ...
    async def create_account(self, tenant_id: str, parent_id: str | None, limit_usd: float, reset_period: str = "monthly") -> None:
        ...
        async with self.connection() as db:
            await db.execute("""
                INSERT INTO budget_accounts (tenant_id, parent_id, limit_micro, reset_period, current_period_start, created_at)
                VALUES (?, ?, ?, ?, ?, ?)
    ...
    async def check_and_debit(self, tenant_id: str, estimated_usd: float, run_id: str, model: str, idempotency_key: str) -> float:
        ...
        async with self.connection() as db:
            ...
            try:
                cursor = await db.execute("SELECT id FROM budget_ledger WHERE idempotency_key = ?", (idempotency_key,))
                if await cursor.fetchone():
                    await db.rollback()
                    return await self._get_remaining_usd_with_db(db, tenant_id)

                cursor = await db.execute("SELECT parent_id, limit_micro, reset_period FROM budget_accounts WHERE tenant_id = ?", (tenant_id,))
                row = await cursor.fetchone()
                if not row:
                    await db.rollback()
                    return 1_000_000.0
                
                parent_id, limit_micro, reset_period = row
                period = self._get_current_period(reset_period)
                ...
                await db.execute("""
                    INSERT INTO budget_ledger (tenant_id, entry_type, amount_micro, run_id, model, idempotency_key, created_at, period)
    ...
    async def _recursive_check_and_update(
        ...
        *,
        ...
        spent, limit, parent_id, reset_period = row
        spent = spent or 0
    ...
    async def adjust_actual(self, tenant_id: str, run_id: str, actual_usd: float, idempotency_key: str) -> None:
        ...
        async with self.connection() as db:
            ...
            try:
                ...
                await db.execute("""
                    INSERT INTO budget_ledger (tenant_id, entry_type, amount_micro, run_id, idempotency_key, created_at, period)
    ...
    async def _recursive_update_balance(
        ...
        *,
        ...
        cursor = await db.execute("SELECT parent_id FROM budget_accounts WHERE tenant_id = ?", (tenant_id,))
        row = await cursor.fetchone()
        if row and row[0]:
            await self._recursive_update_balance(
                db, row[0], amount_micro, period, visited=visited
            )

    async def _get_remaining_usd_with_db(self, db, tenant_id: str) -> float:
        cursor = await db.execute("SELECT reset_period, limit_micro FROM budget_accounts WHERE tenant_id = ?", (tenant_id,))
        row = await cursor.fetchone()
        if not row: return 1_000_000.0
        
        period_type, limit_micro = row
        period = self._get_current_period(period_type)
        
        cursor = await db.execute("SELECT spent_micro FROM budget_balances WHERE tenant_id = ? AND period = ?", (tenant_id, period))
    ...
    async def get_state(self, tenant_id: str) -> tuple[BudgetState, BudgetConfig] | None:
        async with self.connection() as db:
            ...
            config = BudgetConfig(
                limit_usd=micro_to_usd(limit_micro),
                reset_period=period_type
            )
            return state, config
...
class TenantBudgetManager:
    ...
    async def reserve(self, tenant_id: str, estimated_cost: float, run_id: str, model: str, idempotency_key: str) -> float:
        return await self._store.check_and_debit(tenant_id, estimated_cost, run_id, model, idempotency_key)

    async def finalize(self, tenant_id: str, run_id: str, actual_cost: float, idempotency_key: str) -> None:
