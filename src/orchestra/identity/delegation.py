...
from orchestra.core.errors import UCANVerificationError, CapabilityDeniedError
...
async def verify_delegation_chain(
    ...
    try:
        ...
        raise UCANVerificationError(f"Failed to parse UCAN header/payload: {str(e)}")
    ...
    if not UCANManager.check_capability(payload, required_capability):
        ...
        raise CapabilityDeniedError(f"UCAN does not grant {required_capability.ability} on {required_capability.resource}")
    ...
    if not proofs:
        ...
        # In a real system, we'd check if 'iss' matches the resource prefix (e.g. did:A owns did:A/tools)
    ...
    if not granted_by_proof:
        raise CapabilityDeniedError(f"Capability {required_capability.resource} not granted by any proofs")
