...
class UCANManager:
    ...
    def issue(
        ...
        if not self._signing_key or not self._issuer_did:
            raise RuntimeError("UCANManager must be initialized with signing_key and issuer_did to issue tokens")
    ...
    def verify(
        ...
        if expected_audience and payload.get("aud") != expected_audience:
            raise UCANVerificationError(f"Audience mismatch: expected {expected_audience}, got {payload.get('aud')}")
    ...
    def check_capability(
        ...
        for entry in att:
            ...
            ability_match = (
                granted_ability == required.ability or
                granted_ability == "*"
            )
